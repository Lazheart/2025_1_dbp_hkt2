...
export const registerUser = async (email: string, passwd: string): Promise<User> => {
  try {
    ...
    if (response.data.status === 200) { // La API de Ahorrista devuelve 200 para registro exitoso
      ...
      localStorage.setItem('user_email', response.data.result.email); // Guarda el email también
      return { email: response.data.result.email, token: response.data.result.token };
    }
