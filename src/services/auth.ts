...
export const registerUser = async (email: string, passwd: string): Promise<User> => {
  try {
    ...
    if (response.data.status === 200) { // La API de Ahorrista devuelve 200 para registro exitoso
      localStorage.setItem('jwt_token', response.data.data.token); // Guarda el token JWT
      localStorage.setItem('user_email', response.data.data.email); // Guarda el email también
      return { email: response.data.data.email, token: response.data.data.token };
    }
